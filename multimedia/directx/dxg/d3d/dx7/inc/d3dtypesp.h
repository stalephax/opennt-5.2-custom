...
#if(DIRECT3D_VERSION >= 0x0500)
// This renderstate value must never be used.
// There is a duplicate define in d3dhal.h that must be kept in sync with this.
    D3DRENDERSTATE_OLDALPHABLENDENABLE   = 42,   /* TRUE to enable alpha blending */
...
// a macro, not part of the enum, so we can do an MSINTERNAL check if it's already
// defined in d3dhal.h, where it has to be defined publicly for drivers' sake.
...
typedef enum _D3DTEXTURESTAGESTATETYPE
{
    ...
    D3DTSS_SHADOWATTENUATION = 25, /* D3DVALUE darkness of shadow, 1.0 means no shadow, 0.0 means completely black shadow */
    D3DTSS_SHADOWZBIASMIN = 26,    /* D3DVALUE minimum amount to offset the shadow Z compare to avoid self-shadowing */
    D3DTSS_SHADOWZBIASMAX = 27,    /* D3DVALUE maximum offset to offset Z for Z compare.  If greater than SHADOWZBIASMIN */
                                   /* a random Z offset will be choosen between MIN and MAX */
    D3DTSS_SHADOWSIZE = 28,        /* D3DVALUE a random U,V jitter amount to control shadow size, expressed */
                                   /* in shadow texture units.  0.0 means crisp shadows. */
#endif /* DIRECT3D_VERSION >= 0x0700 */
// D3DTSS_MAX is defined in d3dhal.h for internal use only.
// It must be kept up to date with this enum
...
#if(DIRECT3D_VERSION >= 0x0700)
    D3DTFG_SHADOW_1 = 6,        // shadow filter sizes
    D3DTFG_SHADOW_2 = 7,
    D3DTFG_SHADOW_3 = 8,
    D3DTFG_SHADOW_4 = 9,
    D3DTFG_SHADOW_5 = 10,
