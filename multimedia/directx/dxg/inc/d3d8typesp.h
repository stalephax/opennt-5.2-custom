...
#define D3DCLEAR_RESERVED0         0x00000008l  /* Defined in D3DHAL.H */
...
// This renderstate is defined for internal use (only) in d3dhal.h
// There is a duplicate define in d3dhal.h that must be kept in sync with this.
...
typedef enum _D3DFORMAT
{
    ...
    /* These are additional explicit Z formats that the driver can report, */
    /* but are not API visible. */
    /* NOTE!:: Must be kept in sync w/ DWORD defns in ddrawi.h and ddrawint.h*/
    /* NOTE!:: These must be kept in sync w/ defns in d3dhal.h */
    D3DFMT_S1D15                = 72,
    D3DFMT_S8D24                = 74,
    D3DFMT_X8D24                = 76,
    D3DFMT_X4S4D24              = 78,
...
typedef enum _D3DSWAPEFFECT
{
    ...
    // Internal types numbered backwards
    D3DSWAPEFFECT_NO_PRESENT        = 0x40000000,
...
#define VALID_POOL(pool)    ((UINT)(pool) <= D3DPOOL_SCRATCH)
#define VALID_INTERNAL_POOL(pool) ((UINT)(pool) <= D3DPOOL_NONLOCALVIDMEM)
#define IS_D3D_ALLOCATED_POOL(Pool) ((Pool == D3DPOOL_SYSTEMMEM) || (Pool == D3DPOOL_SCRATCH))
...
typedef enum _D3DRESOURCETYPE {
    ...
    // Internal types numbered backwards from 255
    D3DRTYPE_IMAGESURFACE           =254,
    D3DRTYPE_COMMANDBUFFER          =255,
...
/* Internal Usages */
// Note: when we turn LoadOnce on again, we should update DPF_EXPLAIN_BAD_LOCK_FLAGS
#define D3DUSAGE_LOADONCE           (0x00000004L)
#define D3DUSAGE_ALPHACHANNEL       (0x01000000L)
#define D3DUSAGE_DISCARD            (0x02000000L)
#define D3DUSAGE_LOCK               (0x04000000L)
#define D3DUSAGE_TEXTURE            (0x08000000L)
#define D3DUSAGE_BACKBUFFER         (0x10000000L)
#define D3DUSAGE_INTERNALBUFFER     (0x20000000L)
#define D3DUSAGE_OFFSCREENPLAIN     (0x40000000L)
#define D3DUSAGE_PRIMARYSURFACE     (0x80000000L)

/* Useful masks for usages */
#define D3DUSAGE_TEXTURE_VALID       (D3DUSAGE_RENDERTARGET    | \
                                      D3DUSAGE_DEPTHSTENCIL    | \
                                      D3DUSAGE_DYNAMIC)

#define D3DUSAGE_CUBETEXTURE_VALID   (D3DUSAGE_TEXTURE_VALID)

#define D3DUSAGE_VOLUMETEXTURE_VALID (D3DUSAGE_DYNAMIC)

/* Flags valid for render-targets and zbuffers */
#define D3DUSAGE_SURFACE_VALID     (D3DUSAGE_RENDERTARGET      | \
                                    D3DUSAGE_DEPTHSTENCIL)

/* Flags valid for vertexbuffers and indexbuffers */
#define D3DUSAGE_VB_VALID          (D3DUSAGE_WRITEONLY         | \
                                    D3DUSAGE_SOFTWAREPROCESSING| \
                                    D3DUSAGE_POINTS            | \
                                    D3DUSAGE_RTPATCHES         | \
                                    D3DUSAGE_NPATCHES          | \
...
#define D3DUSAGE_IB_VALID          (D3DUSAGE_VB_VALID)
...
#define VALID_CUBEMAP_FACETYPE(face)    ((UINT)face < 6)

/* Lock flags */
/* These values are chosen so that people who pass */
/* DDLOCK_ flags aren't broken; also so we don't have */
/* to translate flags to the driver */
...
/* This is a new flag for DX8*/
#define D3DLOCK_NO_DIRTY_UPDATE     0x00008000L

#define D3DLOCK_VALID               (D3DLOCK_READONLY         | \
                                     D3DLOCK_DISCARD           | \
                                     D3DLOCK_NOOVERWRITE      | \
                                     D3DLOCK_NOSYSLOCK         | \
                                     D3DLOCK_NO_DIRTY_UPDATE)

#define D3DLOCK_SURF_VALID          (D3DLOCK_READONLY         | \
                                     D3DLOCK_NOSYSLOCK         | \
                                     D3DLOCK_NO_DIRTY_UPDATE)

#define D3DLOCK_VOL_VALID           (D3DLOCK_SURF_VALID)
...
typedef struct _D3DBUFFER_DESC
{
    D3DFORMAT           Format;
...
/* Length must be same as MAX_DDDEVICEID_STRING */
